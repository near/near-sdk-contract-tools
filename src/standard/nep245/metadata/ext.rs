...
pub trait Nep245Metadata {
    /// Returns the top-level contract metadata.
    ...
    fn mt_metadata_base_by_token_id(&self, token_ids: Vec<TokenId>) -> Vec<BaseMetadata>;
    /// Returns a list of base metadata by their IDs.
    fn mt_metadata_base_by_metadata_id(
        &self,
        base_metadata_ids: Vec<BaseMetadataId>,
    ) -> Vec<BaseMetadata>;
}
